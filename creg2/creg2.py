...
def fit_model(lbl, lbl_feat, out_dim, in_dim, X, Y, N, write_model=None, l1=1e-5):
    assert len(X) == len(N)
    assert len(Y) == len(X)
    model = IOLogisticRegression()
    model.fit(in_dim, out_dim, X, N, Y, lbl_feat, len(lbl), iterations=3000, minibatch_size=20, l1=l1, write=True)
...
if args.tx is not None and args.ty is not None:
    model = fit_model(labels, label_features, out_dim, in_dim, X, Y, N, 'model_output')
